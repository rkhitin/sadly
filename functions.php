...
function disqus_embed($disqus_shortname = 'ronhit') {
    global $post;
...
function oriolo_pagination($pages = '', $range = 2)
{
 ...
 class CSS_Menu_Maker_Walker extends Walker {
  ...
  function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
    ...
    $dropDownMenuClass = ($item->hasChildren) ? ' dropdown' : '';
    $class_names = $class_names ? ' class="' . esc_attr( $class_names ) . $dropDownMenuClass .  '"' : '';
    ...
    if ($item->hasChildren) {
        $item_output .= ' <i class="fa fa-arrow-down dropdown-arrow"></i>';
    }
    $item_output .= '</span></a>';
